...
_LEXICAL_SCHEMA_VERSION: Final[str] = "1"
...
class DuckDBBm25Store(LexicalStore):
    ...
    def setup(self) -> None:
        """Create lexical index tables and supporting indexes."""
...
class DuckDBLexicalRetriever:
    ...
    def retrieve(
        ...
        *,
        ...
        chunks = self._reader.get_chunks_by_ids([chunk_id for chunk_id, _ in hits])
        scores: dict[str, float] = dict(hits)
        return [
            SearchResult(
                path=chunk.path,
